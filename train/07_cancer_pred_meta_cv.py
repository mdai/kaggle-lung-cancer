...
Xy_meta_train_df = stage1_labels.copy()
...
X_meta_test_df = stage1_sample_submission.copy()
...
def run(folds):
    ...
    meta_model_kfold_list_name = 'ens_1_750_meta_model_{}fold_list'.format(folds)
    ...
    cancer_preds_test = stage1_sample_submission.copy().sort_values(by='id')
    cancer_preds_test['cancer'] = y_pred_comb_test
    output_pred_filename = 'ens_1_750_meta_model_{}fold_cv_comb_cancer_preds_test'.format(folds)
    cancer_preds_test.to_csv('../predictions/{}.csv'.format(output_pred_filename), index=False)
